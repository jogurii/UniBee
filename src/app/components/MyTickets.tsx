      ...
      <header className="px-6 pt-8 pb-6 bg-[#0B1120] rounded-b-[2.5rem] shadow-lg relative z-20">
        ...
        <div className="flex bg-white/5 p-1.5 rounded-2xl border border-white/10 relative">
          <button
            ...
            className={`touch-target flex-1 py-3 text-sm font-bold rounded-xl transition-all relative z-10 ${activeTab === "Upcoming" ? "text-white" : "text-slate-400"}`}
          >
            Akan Datang
          </button>
          <button
            onClick={() => setActiveTab("Past")}
            className={`touch-target flex-1 py-3 text-sm font-bold rounded-xl transition-all relative z-10 ${activeTab === "Past" ? "text-white" : "text-slate-400"}`}
          >
            Selesai
          </button>
          <motion.div
            className="absolute top-1.5 bottom-1.5 bg-gradient-to-r from-orange-600 to-amber-500 rounded-xl z-0"
            animate={{ x: activeTab === "Upcoming" ? "0%" : "96%", width: "50%" }}
      ...
      <AnimatePresence>
        {selectedTicket && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[60] bg-white dark:bg-[#0B1120] flex flex-col items-center justify-center p-4 sm:p-8 overflow-hidden"
          >
            <button
            ...
            >
              <div className="flex flex-col items-center justify-center flex-1 w-full">
                ...
                <p className="text-slate-500 dark:text-slate-400 font-medium text-sm mb-1">{selectedTicket.location}</p>
                <p className="text-slate-500 dark:text-slate-400 font-medium text-xs mb-4">{selectedTicket.date} • {selectedTicket.time} - {selectedTicket.endTime}</p>
                ...
                <p className="text-xs font-black text-slate-400 tracking-[0.2em] mb-3 uppercase">{selectedTicket.ticketCode}</p>
              </div>
                ...
                {selectedTicket.status === "Upcoming" ? (
                  <>
                    ...
                    <button
                      ...
                      className="text-red-500 font-bold text-sm hover:bg-red-50 px-4 py-3 rounded-xl transition-colors"
                    >
                      Batalkan RSVP
                    </button>
                  </>
      ...
      <AnimatePresence>
        {showToast && (
          <motion.div
            ...
            className="fixed bottom-32 left-1/2 -translate-x-1/2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-6 py-4 rounded-full shadow-2xl flex items-center gap-3 z-[100] border border-slate-800 dark:border-slate-200 min-w-[280px] justify-center"
          >
            <CheckCircle2 className="w-5 h-5 text-emerald-500" />
            <span className="text-sm font-bold">RSVP Berhasil Dibatalkan</span>
          </motion.div>
        )}
      </AnimatePresence>
